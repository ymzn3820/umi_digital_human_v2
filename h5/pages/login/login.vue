<template>
	<view id="phone">
		...
		<view class="loginInput">
			<u-input @blur='isMobile' placeholder="请输入手机号码" border="bottom" v-model="phone" color='#000'>
		...
		<view class="agree_text_right" style="margin-top: 20rpx;" @click="to_sign_up">还没有账号？立即注册</view>
	...
	export default {
		...
		methods: {
			isMobile() {
				...
				const that = this
				isMobile(data).then(res => {
					if (res.code == 20000) {
						uni.showModal({
							content: '此号码还未注册，请先进行注册',
							showCancel: false,
							success: function(res) {
								if (res.confirm) {
									uni.navigateTo({
										url: '/pages/login/sign_up?phone=' + that.phone
									})
								} else if (res.cancel) {
									console.log('用户点击取消');
								}
