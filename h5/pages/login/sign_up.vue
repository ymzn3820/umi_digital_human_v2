<template>
	<view id="phone">
		...
		<view class="title">欢迎来到短视频数字人平台</view>
		<view class="loginInput">
			...
			<u-input placeholder="请输入邀请码" border="bottom" v-model="InvitationCode" color='#000'>
				<u--text text="邀请码" slot="prefix" margin="0 3px 0 0" type="info"></u--text>
			</u-input>
		</view>
		<u-button color="#1F64FF" :customStyle="{'height':'48px'}" @click="goToLogin">注册</u-button>
		<view class="agree_text_right" style="margin-top: 20rpx;" @click="toLogin">已有账号？立即登录</view>
		...
		<view class="agree">
			<view style="display: flex; align-items: center; justify-content: center;">
				...
				<text class="agree_text">注册代表同意</text>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			console.log(option)
			if (option.company_code) { //企业别邀请成员第一次注册会执行以下代码
		...
		methods: {
			...
			goToLogin() {
				...
				uni.request({
					url: global.loginUrl + '/api/user/mobile_register',
					data: val,
					header: headers,
					method: 'POST',
					success: (res) => {
						console.log(res, 1147);
