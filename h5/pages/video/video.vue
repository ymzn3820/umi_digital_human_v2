<template>
	<view>
		...
		<view v-if="video_type == 1">
			<scroll-view class="list_ul" scroll-y="true" v-if="infoList.length > 0">
				<view class="cont_list" v-for="(item, index) in infoList" :key="index">
					<view class="list_li" @click="toRoal(item)">
						...
						<view class="li_text">
							...
							<view class="text_r" v-if="isEdit">
								<image class="r_img" :src="uncheckImg" v-if="!item.isCheck"
									@click.stop="selectDel(index)"></image>
								<image class="r_img" :src="checkImg" v-if="item.isCheck"
									@click.stop="unSelectDel(index)"></image>
							</view>
						</view>
			...
			<view class="on_order" style="display: block;" v-if="infoList.length == 0">
				<view class="ul_style" @click="toCreated()">
					<view class="li_style">
						<view class="li_wz">
					...
					<view class="list_li">
						...
						<view class="li_text">
							<view class="text_l">
								...
								<view class="li_status" style="background: rgba(31, 100, 255, 0.10); color: #f9ae3d"
									v-if="item.make_status == 1">队列中</view>
								<view class="li_status" style="background: rgba(31, 100, 255, 0.10); color: #1F64FF"
									v-if="item.make_status == 2">制作中</view>
								<view class="li_status" style="background: rgba(50, 190, 72, 0.10); color: #32BE48"
									v-if="item.make_status == 3">制作成功</view>
								<view class="li_status" style="background: rgba(247, 68, 57, 0.10); color: #F74439"
									v-if="item.make_status == 4">制作失败</view>
							</view>
							<view class="text_r" v-if="item.make_status > 2">
								<image class="more_img" :src="moreImg"
									@click.stop="selectMake(index, item.make_status)"></image>
							</view>
						</view>
		...
		<view class="cont_btn" v-if="isEdit && video_type == 1 && infoList.length > 0">
	...
	export default {
		...
		methods: {
			...
			checkVideo() {
				...
				uni.navigateTo({
					url: '/pages/figure/play?video_url=' + video_url,
				})
			},
			toIdea() {
				uni.navigateTo({
