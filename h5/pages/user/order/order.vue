		...
		<view class="top_cont">
			<view class="search_cont">
				<view class="cont_tabs">
					<view class="tabs_l" style="width: 12%;">
						<view @click="version(0)" :class="[active == 0 ? 'active_tabs' : 'tabs']">全部</view>
					</view>
					<view class="tabs_l">
						<view @click="version(1)" :class="[active == 1 ? 'active_tabs' : 'tabs']">形象克隆</view>
					</view>
					<view class="tabs_l">
						<view @click="version(2)" :class="[active == 2 ? 'active_tabs' : 'tabs']">声音克隆</view>
					</view>
					<view class="tabs_l">
						<view @click="version(4)" :class="[active == 4 ? 'active_tabs' : 'tabs']">视频生成</view>
					</view>
					<view class="tabs_l" style="width: 25%;">
						<view @click="version(5)":class="[active == 5 ? 'active_tabs' : 'tabs']" >平台通用功能</view>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	import {getExchange} from '@/apis/home.js'
	export default {
		data() {
			return {
				active: 0,
		...
		methods: {
			getExchange(){
				console.log('进来了');
				let data={
					activate_type_id:this.active
				}
				getExchange(data).then(res=>{
					console.log(res,1458);
				}).catch(err=>{
					console.log(err,1524);
					this.$api.msg('获取失败')
				})
			},
			// 选择版本
