	...
	<view>
		<view class="containar">
			<view class="edit_btn">
				...
				<view class="btn_upload" @click="submit(1)">导出视频</view>
			</view>
			...
			<view class="video_add">
				<view class="add_li">
					<view class="li_l">
						<view class="link_l">
							<image :src="getImgUrl(infoDetail.video_cover_url)" alt="" mode="aspectFit" class="l_img">
							</image>
						</view>
					</view>
				...
				<view v-for="(item, index) in textList" :key="index" v-if="typeIndex == 1">
					<view class="li_textare">
						<u--textarea v-model="item.live_script" placeholder="请输入内容" style="background: #F9F9F9;"
					...
					<view class="ul_li">
						<view class="ul_style" v-for="(item, index) in infoList" :key="index">
							<view class="li_style" @click="toRoal(index)">
								<view class="li_wz">
									<image :class="select_index === index ? 'li_img_active' : 'li_img'" mode="aspectFit"
										:src="getImgUrl(item.video_cover_url)"></image>
								</view>
		...
		<!-- 更换标题 -->
		<u-modal :show="showTitle" @cancel="showTitle = false" confirmText="确认" confirmColor="#1F64FF"
			:showCancelButton='true' @confirm="changeKey">
			<view class="key_cont" style="">
				...
				<u-input :customStyle='{"padding":"8px", "width":"100%"}' inputAlign="center" v-model="messageTitle"
					placeholder="请输入标题" type="text" maxlength="10"></u-input>
			</view>
		</u-modal>

		<!-- 驱动类型选择 -->
		<u-picker :show="typeShow" :columns="typeList" v-model="typeIndex" keyName="label" @cancel="typeShow = false"
			@confirm="typeSubmit"></u-picker>

		<!-- 声音模型类型选择 -->
		<u-picker :show="voiceShow" :columns="selectVoice" v-model="voiceIndex" keyName="voice_name"
			@cancel="voiceShow = false" @confirm="voiceSubmit"></u-picker>
	...
	export default {
		...
		methods: {
			...
			postDuration() {
				...
				postDuration(data).then(res => {
					if (res.code == 20000) {
						uni.switchTab({
							url: '/pages/video/video'
						})
					} else if (res.code == 40040) {
						uni.showModal({
							content: '无可用时长， 请先兑换卡密',
							confirmText: '兑换卡密',
							success: (res) => {
								if (res.confirm) {
									this.show = true
						...
						uni.uploadFile({
							...
							success: (uploadFileRes) => {
								...
								if (imgData.code == 20000) {
									...
									let url_val = {
										live_sound_url: imgData.data.new_url,
										sound_name: name
									}
			...
			getCode() { // 非静默授权，第一次有弹框
				let local = global.url + '/#/pages/figure/figure_oral' //当前页面的地址
