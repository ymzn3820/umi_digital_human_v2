...
<script>
	...
	export default {
		...
		methods:{
			...
			toEquity(){
				if(uni.getStorageSync('checkToken')) {
					...
					checkToken(val).then(res => {
						if (res.code == 20000) {
							uni.navigateTo({
								url:'/childCont/facilitator/equity'
							})
						} else if(res.code == 40023) {
							uni.navigateTo({
								url:'/childCont/facilitator/enter'
							})
						} else {
			...
			toUpgrades(){
				uni.navigateTo({
					url:'/childCont/distribution/grade'
				})
			},
			// 服务商信息
			toService(){
				uni.navigateTo({
					url:'/childCont/facilitator/message'
				})
				// uni.navigateTo({
				// 	url:'/childCont/facilitator/service'
				// })
			}
		}
	}
