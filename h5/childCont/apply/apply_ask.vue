	...
	<view class="containar">
				...
				<view v-for="(item, index) in questionList" :key="index">
					...
					<view class="chat_text" v-if="item.info_type_name == 'Textarea'">
						<view class="text_cont" style='display: flex;justify-content: space-between;'>
							...
							<view  style="display: flex; align-items: center;" v-if="index == 0">
								<view class="type_text">引擎：</view>
									<text class="l_text" style="margin-right: 10upx;">{{ chatType }}</text>
								<!-- <view class="l_chat">
									<u-icon name="arrow-down" color="#DBDBDB" size="14"></u-icon>
		...
		<view >
			...
			<u-picker :show="chatShow" :columns="samplerList" :closeOnClickOverlay="true" @close="chatShow = false"
				v-model="chatType" keyName="label" @cancel="chatShow = false" @confirm="version"></u-picker>
		</view>
	...
	export default {
		data() {
			return {
				...
				evaluateType7: [],
				//腾讯混元
				controller_eleven: null, // 用于终止会话
				t_time_eleven: null, // 计时器
				eleven_shi: null,
				evaluateType8: [],
				//火山云雀
				controller_twelve: null, // 用于终止会话
				t_time_twelve: null, // 计时器
				twelve_shi: null,
				evaluateType9: [],
		   ...
		   version(e) {
			...
			stopChat() {
				if(this.listName == 'three') {
					...
					if (this.controller_eleven != null) {
						this.controller_eleven.abort()
					}
					this.messageList[this.listName][this.messageList[this.listName].length - 1].loading = true
					clearInterval(this.t_time_eleven)
					clearInterval(this.eleven_shi)
					...
					if (this.controller_twelve != null) {
						this.controller_twelve.abort()
					}
					this.messageList[this.listName][this.messageList[this.listName].length - 1].loading = true
					clearInterval(this.t_time_twelve)
					clearInterval(this.twelve_shi)
			...
			async showFun() {
				...
				if(this.listName == 'three' || this.listName == 'five' || this.listName == 'six' || this.listName == 'seven'|| this.listName == 'eleven'|| this.listName == 'twelve'|| this.listName == 'eight') {
